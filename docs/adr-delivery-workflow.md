...
- Близость к реальному сценарию Bitrix24.
- Локальный SQLite - для идемпотентности, retry state и дедупликации задач.
- Между webhook и worker "лежит" RabbitMQ для асинхронной обработки, гарантии доставки и управления нагрузкой.
