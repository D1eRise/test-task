...
Нужно автоматизировать переход сделки в `DELIVERY` через Bitrix24 REST без UI и без лишнего каркаса. Обязательные части сценария:
...
Ограничение по времени небольшое, поэтому решение намеренно собрано как простой сервис, а не как отдельная платформа.
...
Для test-task это достаточно: БП либо реально доступен в портале, либо корректно отключён без падения основного потока.
