...
PHP 8.3 сервис под тестовое задание: входящий webhook `ONCRMDEALUPDATE` попадает в RabbitMQ, сервис берёт `FIELDS[ID]`, дочитывает текущую сделку через Bitrix24 REST, фильтрует нужную категорию, определяет реальный вход в `DELIVERY` по локальному snapshot стадии и fallback через `crm.stagehistory.list`, а worker читает сделку, контакт и товарные позиции напрямую из Bitrix24 REST, вызывает courier mock, считает `RiskScore`, пишет результат обратно в сделку, добавляет запись в timeline и создаёт задачу (без дублей).

Для идемпотентности, ретраев и дедупа задач используется SQLite state-store (достаточно для тестового непрод-проекта).

Живой прогон против реального портала Bitrix24 не выполнялся, поскольку в рамках тестового задания не были предоставлены портал и доступы (развернуть свой не представилось возможным в рамках жёстких ограничений на время выполнения задания (2 вечера (ср, чт) по 3 часа после работы и 3 часа днём в пятницу, портатить больше не мог физически)). Из-за этого end-to-end работа интеграции с настоящим Bitrix24 остаётся непроверенной на практике. При этом в проекте реализован Bitrix24 REST-клиент, а текущие unit-тесты и локальная эмуляция 200 событий покрывают основную бизнес-логику, идемпотентность, очередь и retry-механику на тестовых данных. Тем не менее без подключения к реальному порталу нельзя полностью исключить возможные интеграционные ошибки и баги, которые проявляются только в живой среде.
